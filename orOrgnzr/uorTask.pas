...
begin
  case tc of
     tcDateCreated        : result := ' «—ÌŒ «ÌÃ«œ';
     tcSubject            : result := '„Ê÷Ê⁄';
     tcDueDate            : result := ' «—ÌŒ ”——”Ìœ';
     tcStartDate          : result := ' «—ÌŒ ‘—Ê⁄';
     tcLastModifyDate     : result := ' «—ÌŒ  €ÌÌ—';
     tcStatus             : result := 'Ê÷⁄Ì ';
     tcImportance         : result := '«Â„Ì ';
     tcPercentCompleted   : result := 'œ—’œ  ﬂ„Ì·';
     tcReminderDate       : result := ' «—ÌŒ Ì«œ¬Ê—Ì';
     tcAttachments        : result := 'ÅÌÊ” ';
     tcCategories         : result := 'ÿ»ﬁÂ »‰œÌ';
     tcPrivacy            : result := '‘Œ’Ì';
     tcDateCompleted      : result := ' «—ÌŒ  ﬂ„Ì·';
     tcActualWork         : result := 'ﬂ«— Ê«ﬁ⁄Ì';
     tcTotalWork          : result := 'ﬂ· ﬂ«—';
     tcContacts           : result := ' „«”Â«';
     tcBillingInformation : result := '«ÿ·«⁄«  ’Ê— Õ”«»';
     tcCompanies          : result := '‘—ﬂ Â«';
     tcMileage            : result := '›«’·Â';
  end;
end;
...
begin
  case tst of
      tsNotStarted: Result:= '‘—Ê⁄ ‰‘œÂ';
      tsInProgress: Result:= 'œ— Õ«· «Ã—«';
      tsCompleted : Result:= ' ﬂ„Ì· ‘œÂ ';
      tsWaiting   : Result:= '„‰ Ÿ— «” ';
      tsDeferred  : Result:= '«—Ã«⁄ ‘œÂ';
  end;
end;
